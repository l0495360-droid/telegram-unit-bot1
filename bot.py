...
PHYSICAL_QUANTITIES = {
    ...
    "Скорость передачи данных": {
        ...
        "килобит/час": 1024.0 / 3600.0,
        "мегабит/час": 1048576.0 / 3600.0,
        "килобайт/час": 8192.0 / 3600.0,
        "мегабайт/час": 8388608.0 / 3600.0,
        "гигабайт/час": 8589934592.0 / 3600.0
    }
}
...
- Древнерусские меры длины (вершок, пядь, локоть и др.)
...
- Скорость
- Давление
- Информация (биты, байты, мегабайты и др.)
- Скорость передачи данных (бит/сек, Мбит/сек и др.)
...
async def select_unit_to(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if category == "Длина":
        ...
        if "байт" in context.user_data['unit_from'] and "бит" in unit_to:
            examples = "\n💡 Пример: 1 байт = 8 бит"
        elif "килобайт" in context.user_data['unit_from'] and "байт" in unit_to:
            examples = "\n💡 Пример: 1 килобайт = 1024 байта"
    elif category == "Древнерусские меры длины":
        if "вершок" in context.user_data['unit_from'] and "метр" in unit_to:
            examples = "\n💡 Пример: 1 вершок = 0.04445 м"
        elif "сажень" in context.user_data['unit_from'] and "метр" in unit_to:
            examples = "\n💡 Пример: 1 сажень = 2.1336 м"
    elif category == "Скорость передачи данных":
        if "мегабит/сек" in context.user_data['unit_from'] and "мегабайт/сек" in unit_to:
            examples = "\n💡 Пример: 100 Мбит/сек = 12.5 МБ/сек"
        elif "килобит/сек" in context.user_data['unit_from'] and "килобайт/сек" in unit_to:
            examples = "\n💡 Пример: 1024 Кбит/сек = 128 КБ/сек"
        elif "мегабит/сек" in context.user_data['unit_from'] and "гигабайт/час" in unit_to:
            examples = "\n💡 Пример: 100 Мбит/сек ≈ 45 ГБ/час"
